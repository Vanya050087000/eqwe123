...
local FIREBASE_URL = "https://octo-39755-default-rtdb.firebaseio.com/keys"
...
local function checkKey(key)
    ...
    local body = httpRequest(FIREBASE_URL .. ".json")
    ...
    for keyId, kd in pairs(data) do
        if kd.key == key then
            ...
            pcall(function()
                httpRequest(FIREBASE_URL .. "/" .. keyId .. ".json", "PATCH", HttpService:JSONEncode(updateData))
            end)
...
local function createKeyGUI()
    ...
    checkBtn.Position = UDim2.new(0, 20, 0, 180)
    ...
    local tween = TweenService:Create(main, TweenInfo.new(0.3, Enum.EasingStyle.Back), {Size = UDim2.new(0, 380, 0, 260)})
    tween:Play()
...
local function doCheck()
    ...
    local key = keyGUI.keyInput.Text:gsub(" ", ""):upper()
    
    if key == "" then
        keyGUI.icon.Text = "⚠️"
        keyGUI.statusText.Text = "Введите ключ!"
        keyGUI.statusText.TextColor3 = Color3.fromRGB(255, 200, 50)
        return
    end
    
    isChecking = true
    keyGUI.checkBtn.Text = "ПРОВЕРКА..."
    keyGUI.checkBtn.BackgroundColor3 = Color3.fromRGB(100, 100, 100)
    keyGUI.icon.Text = "⏳"
    keyGUI.statusText.Text = "Проверяю..."
    keyGUI.statusText.TextColor3 = Color3.fromRGB(200, 200, 200)
    
    task.wait(0.5)
    
    local ok, msg, keyData = checkKey(key)
    
    if ok then
        keyGUI.icon.Text = "✅"
        keyGUI.statusText.Text = "Доступ разрешен!"
        keyGUI.statusText.TextColor3 = Color3.fromRGB(50, 255, 100)
        keyGUI.checkBtn.Text = "✅ ДОСТУП ОТКРЫТ"
        keyGUI.checkBtn.BackgroundColor3 = Color3.fromRGB(40, 200, 80)
        ...
        keyGUI.icon.Text = "❌"
        keyGUI.statusText.Text = msg
        keyGUI.statusText.TextColor3 = Color3.fromRGB(255, 70, 70)
        keyGUI.checkBtn.Text = "ПРОВЕРИТЬ"
        keyGUI.checkBtn.BackgroundColor3 = Color3.fromRGB(88, 101, 242)
...
keyGUI.checkBtn.MouseButton1Click:Connect(doCheck)

keyGUI.keyInput.InputBegan:Connect(function(input)
