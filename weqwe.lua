...
local Library do 
    ...
    Library = {
        ...
        KeySystem = {
            ...
            KeyData = nil,
            HWID = nil,
            FIREBASE_URL = "https://octo-39755-default-rtdb.firebaseio.com/keys"
        }
    }
    ...
    function Library:GetHWID()
        ...
        pcall(function()
            parts[1] = game:GetService("RbxAnalyticsService"):GetClientId()
        end)
        
        pcall(function()
            parts[2] = tostring(LocalPlayer.UserId)
        end)
        
        pcall(function()
            parts[3] = identifyexecutor and identifyexecutor() or "unknown"
        end)
        
        local hwid = table.concat(parts, "_")
        
        local hash = 0
        for i = 1, #hwid do
            hash = (hash * 31 + string.byte(hwid, i)) % 4294967296
        end
        
        self.KeySystem.HWID = string.format("%08X", hash)
        return self.KeySystem.HWID
    end

    function Library:HTTPRequest(url, method, body)
        method = method or "GET"
        
        if syn and syn.request then
            local r = syn.request({Url = url, Method = method, Headers = {["Content-Type"] = "application/json"}, Body = body})
            return r.Body
        elseif request then
            local r = request({Url = url, Method = method, Headers = {["Content-Type"] = "application/json"}, Body = body})
            return r.Body
        elseif http and http.request then
            local r = http.request({Url = url, Method = method, Headers = {["Content-Type"] = "application/json"}, Body = body})
            return r.Body
        end
        return nil
    end

    function Library:CheckKey(key)
        local hwid = self:GetHWID()
        
        local body = self:HTTPRequest(self.KeySystem.FIREBASE_URL .. ".json")
        if not body or body == "null" then
            return false, "Сервер недоступен", nil
        end
        
        local data = HttpService:JSONDecode(body)
        
        for keyId, kd in pairs(data) do
            if kd.key == key then
                if not kd.active then
                    return false, "Ключ заблокирован", nil
                end
                
                if kd.expires then
                    local y, m, d = kd.expires:match("(%d+)-(%d+)-(%d+)")
                    if y then
                        local expTime = os.time({year=tonumber(y), month=tonumber(m), day=tonumber(d), hour=23, min=59})
                        if os.time() > expTime then
                            return false, "Ключ истек", nil
                        end
                    end
                end
                
                local boundHWID = kd.hwid
                
                if boundHWID and boundHWID ~= "" then
                    if boundHWID ~= hwid then
                        return false, "Ключ привязан к другому ПК", nil
                    end
                end
                
                local maxDev = kd.maxDevices or 1
                local hwidList = kd.hwidList or {}
                
                if not boundHWID or boundHWID == "" then
                    if #hwidList >= maxDev then
                        return false, "Лимит устройств исчерпан", nil
                    end
                end
                
                local maxUses = kd.maxUses or 0
                local totalUses = kd.totalUses or 0
                if maxUses > 0 and totalUses >= maxUses then
                    return false, "Лимит использований исчерпан", nil
                end
                
                local updateData = {
                    totalUses = totalUses + 1,
                    lastUsed = os.date("%Y-%m-%d %H:%M:%S"),
                    lastUser = LocalPlayer.Name
                }
                
                if not boundHWID or boundHWID == "" then
                    updateData.hwid = hwid
                    updateData.hwidBoundAt = os.date("%Y-%m-%d %H:%M:%S")
                end
                
                local newList = {}
                local exists = false
                for _, h in ipairs(hwidList) do
                    table.insert(newList, h)
                    if h == hwid then exists = true end
                end
                if not exists then
                    table.insert(newList, hwid)
                end
                updateData.hwidList = newList
                updateData.usedDevices = #newList
                
                self:SafeCall(function()
                    self:HTTPRequest(self.KeySystem.FIREBASE_URL .. "/" .. keyId .. ".json", "PATCH", HttpService:JSONEncode(updateData))
                end)
                
                self.KeySystem.IsActivated = true
                self.KeySystem.KeyData = kd
                
                return true, kd.name, kd
            end
        end
        
        return false, "Ключ не найден", nil
    end

    function Library:ShowKeyGUI()
        if self.KeyGUI then
            self.KeyGUI.Visible = true
            return
        end
        ...
        Main.Name = "Main"
        Main.Size = UDim2New(0, 380, 0, 300)
        Main.Position = UDim2New(0.5, -190, 0.5, -150)
        Main.BackgroundColor3 = FromRGB(43, 43, 43)
        Main.BorderSizePixel = 2
        Main.BorderColor3 = FromRGB(0, 0, 0)
        Main.Parent = Gui
        ...
        Inline.BackgroundColor3 = FromRGB(12, 12, 12)
        Inline.BorderSizePixel = 2
        Inline.BorderColor3 = FromRGB(68, 68, 68)
        Inline.Parent = Main

        local InlineStroke = Instance.new("UIStroke")
        InlineStroke.Color = FromRGB(0, 0, 0)
        ...
        Liner.Name = "Liner"
        Liner.Size = UDim2New(1, 0, 0, 2)
        Liner.Position = UDim2New(0, 0, 0, 0)
        Liner.BackgroundColor3 = FromRGB(31, 226, 130)
        Liner.BorderSizePixel = 0
        Liner.Parent = Inline

        local LinerGradient = Instance.new("UIGradient")
        LinerGradient.Rotation = 90
        LinerGradient.Color = RGBSequence{RGBSequenceKeypoint(0, FromRGB(255, 255, 255)), RGBSequenceKeypoint(1, FromRGB(125, 125, 125))}
        LinerGradient.Parent = Liner

        local Title = Instance.new("TextLabel")
        Title.Name = "Title"
        Title.Size = UDim2New(1, 0, 0, 35)
        Title.Position = UDim2New(0, 0, 0, 10)
        Title.BackgroundTransparency = 1
        Title.Text = "🔐 RB Key System"
        Title.TextColor3 = FromRGB(180, 180, 180)
        Title.TextSize = 16
        ...
        Icon.Name = "Icon"
        Icon.Size = UDim2New(0, 60, 0, 60)
        Icon.Position = UDim2New(0.5, -30, 0, 55)
        ...
        Status.Name = "Status"
        Status.Size = UDim2New(1, -40, 0, 20)
        Status.Position = UDim2New(0, 20, 0, 125)
        Status.BackgroundTransparency = 1
        Status.Text = "Введите ключ для активации"
        Status.TextColor3 = FromRGB(180, 180, 180)
        Status.TextSize = 13
        Status.FontFace = Library.Font
        Status.TextXAlignment = Enum.TextXAlignment.Center
        Status.Parent = Inline

        local KeyInput = Instance.new("TextBox")
        KeyInput.Name = "KeyInput"
        KeyInput.Size = UDim2New(1, -40, 0, 38)
        KeyInput.Position = UDim2New(0, 20, 0, 155)
        KeyInput.BackgroundColor3 = FromRGB(63, 63, 63)
        KeyInput.Text = ""
        KeyInput.PlaceholderText = "RB-XXXX-XXXX"
        KeyInput.PlaceholderColor3 = FromRGB(145, 145, 145)
        KeyInput.TextColor3 = FromRGB(180, 180, 180)
        KeyInput.TextSize = 14
        KeyInput.FontFace = Library.Font
        KeyInput.BorderSizePixel = 0
        KeyInput.Parent = Inline

        local InputStroke = Instance.new("UIStroke")
        InputStroke.Color = FromRGB(0, 0, 0)
        InputStroke.LineJoinMode = Enum.LineJoinMode.Miter
        InputStroke.ApplyStrokeMode = Enum.ApplyStrokeMode.Border
        InputStroke.Parent = KeyInput

        local InputGradient = Instance.new("UIGradient")
        InputGradient.Rotation = 90
        InputGradient.Color = RGBSequence{RGBSequenceKeypoint(0, FromRGB(255, 255, 255)), RGBSequenceKeypoint(1, FromRGB(127, 127, 127))}
        InputGradient.Parent = KeyInput

        local CheckBtn = Instance.new("TextButton")
        CheckBtn.Name = "CheckBtn"
        CheckBtn.Size = UDim2New(1, -40, 0, 35)
        CheckBtn.Position = UDim2New(0, 20, 0, 205)
        CheckBtn.BackgroundColor3 = FromRGB(63, 63, 63)
        CheckBtn.Text = "ПРОВЕРИТЬ"
        CheckBtn.TextColor3 = FromRGB(180, 180, 180)
        CheckBtn.TextSize = 13
        CheckBtn.FontFace = Library.Font
        CheckBtn.BorderSizePixel = 0
        CheckBtn.AutoButtonColor = false
        CheckBtn.Parent = Inline

        local BtnStroke = Instance.new("UIStroke")
        BtnStroke.Color = FromRGB(0, 0, 0)
        ...
        BtnGradient.Color = RGBSequence{RGBSequenceKeypoint(0, FromRGB(255, 255, 255)), RGBSequenceKeypoint(1, FromRGB(127, 127, 127))}
        BtnGradient.Parent = CheckBtn

        local CloseBtn = Instance.new("TextButton")
        CloseBtn.Name = "CloseBtn"
        CloseBtn.Size = UDim2New(0, 25, 0, 25)
        CloseBtn.Position = UDim2New(1, -35, 0, 8)
        CloseBtn.BackgroundTransparency = 1
        CloseBtn.Text = "✕"
        CloseBtn.TextColor3 = FromRGB(180, 180, 180)
        CloseBtn.TextSize = 14
        CloseBtn.FontFace = Library.Font
        CloseBtn.BorderSizePixel = 0
        CloseBtn.AutoButtonColor = false
        CloseBtn.Parent = Inline
        ...
        local function doCheck()
            ...
            local key = KeyInput.Text:gsub(" ", ""):upper()
            
            if key == "" then
                Icon.Text = "⚠️"
                Status.Text = "Введите ключ!"
                Status.TextColor3 = FromRGB(255, 200, 50)
                return
            end
            
            isChecking = true
            CheckBtn.Text = "ПРОВЕРКА..."
            CheckBtn.BackgroundColor3 = FromRGB(100, 100, 100)
            Icon.Text = "⏳"
            Status.Text = "Проверяю..."
            Status.TextColor3 = FromRGB(200, 200, 200)
            
            task.wait(0.5)
            
            local ok, msg, keyData = self:CheckKey(key)
            
            if ok then
                Icon.Text = "✅"
                Status.Text = "Доступ разрешен!"
                Status.TextColor3 = FromRGB(50, 255, 100)
                CheckBtn.Text = "✅ ДОСТУП ОТКРЫТ"
                CheckBtn.BackgroundColor3 = FromRGB(40, 200, 80)
                
                self:SafeCall(function()
                    game:GetService("StarterGui"):SetCore("SendNotification", {
                        Title = "RB Keys",
                        Text = "✅ Доступ разрешен! " .. msg,
                        Duration = 5
                    })
                end)
                
                print("✅ Доступ разрешен! " .. msg)
                
                self.KeySystem.OnActivate and self.KeySystem.OnActivate()
                ...
                Icon.Text = "❌"
                Status.Text = msg
                Status.TextColor3 = FromRGB(255, 70, 70)
                CheckBtn.Text = "ПРОВЕРИТЬ"
                CheckBtn.BackgroundColor3 = FromRGB(63, 63, 63)
        ...
        KeyInput.InputBegan:Connect(function(input)
    ...
    function Library:InitKeySystem(onActivate)
        self.KeySystem.OnActivate = onActivate or function()
...
Library:InitKeySystem(function()
    ...
    Section:Button({
        Name = "Test Button",
        Callback = function()
            Library:Notification("Test button clicked!", 3, Color3.fromRGB(0, 255, 0))
        end
    })
end)
